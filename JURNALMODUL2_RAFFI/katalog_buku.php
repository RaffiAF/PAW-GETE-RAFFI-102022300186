...
$search = isset($_GET['search']) ? trim($_GET['search']) : '';
    ...
    if (!empty($search) && strlen($search) < 3) {
        echo "<script>alert('Pencarian harus minimal 3 karakter.'); window.history.back();</script>";
        exit;
    }
    ...
    if (!empty($search) && !preg_match("/^[a-zA-Z0-9 ]+$/", $search)) {
        echo "<script>alert('Pencarian hanya boleh mengandung huruf dan angka.'); window.history.back();</script>";
        exit;
    }
...
<body>
    ...
    <div class="container mt-5">
        ...
        <form action="katalog_buku.php" method="GET" class="form-inline">
            ...
            <input class="form-control mr-sm-2" type="search" name="search" placeholder="Search" aria-label="Search" value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
